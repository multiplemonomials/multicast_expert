...
if sys.version_info < (3, 12, 0):
    if platform.system() == "Windows":
        if not hasattr(socket, "IP_ADD_SOURCE_MEMBERSHIP"):
            socket.IP_ADD_SOURCE_MEMBERSHIP = 15  # type: ignore[misc]
        if not hasattr(socket, "IP_DROP_SOURCE_MEMBERSHIP"):
            socket.IP_DROP_SOURCE_MEMBERSHIP = 16  # type: ignore[misc]
    elif platform.system() == "Darwin":
        if not hasattr(socket, "IP_ADD_SOURCE_MEMBERSHIP"):
            socket.IP_ADD_SOURCE_MEMBERSHIP = 70  # type: ignore[misc]
        if not hasattr(socket, "IP_DROP_SOURCE_MEMBERSHIP"):
            socket.IP_DROP_SOURCE_MEMBERSHIP = 71  # type: ignore[misc]
    else:  # Assume Linux/Unix
        if not hasattr(socket, "IP_ADD_SOURCE_MEMBERSHIP"):
            socket.IP_ADD_SOURCE_MEMBERSHIP = 39  # type: ignore[misc]
        if not hasattr(socket, "IP_DROP_SOURCE_MEMBERSHIP"):
            socket.IP_DROP_SOURCE_MEMBERSHIP = 40  # type: ignore[misc]
